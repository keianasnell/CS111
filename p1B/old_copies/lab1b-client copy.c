...
//    };
    ...
    while(1){
        ...
        if (ret < 0) {
            fprintf(stderr, "Error in starting poll: %s\n", strerror(errno));
        }
        ...
        if (polls[0].revents & POLLIN) {
            ...
            if (num == 0) {
                fprintf(stderr, "Error: empty buffer: %s\n", strerror(errno));
                exit(0);
            }
            int i;
            for (i = 0; i < num; i++) {
    ...
    while((c = getopt_long(argc, argv, "p:e:l:", long_options, NULL)) != -1){
        switch(c){
            ...
            case 'e':
                ...
                crypt_flag = 1;
//                char *key = get_key("my.key");
//                init_crypt(key, key_len);
            case 'l':
                //If this option is specified, all data written to or read from the server should be logged to the specified file.
                log_flag = 1;
                FILE *fptr;
                fptr = fopen(optarg, "r+");
