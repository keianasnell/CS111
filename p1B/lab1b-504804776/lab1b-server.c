...
void init_crypt(char* key, int key_len){
    ...
    VI = malloc(mcrypt_enc_get_iv_size(decrypt));
    for (i = 0; i < mcrypt_enc_get_iv_size(decrypt); i++) {
        VI[i] = 0;
    }
    if(mcrypt_generic_init(decrypt, key, key_len, VI) < 0) {
    ...
    while((c = getopt_long(argc, argv, "p:e:", long_options, NULL)) != -1){
        switch(c){
            ...
            case 'e':
                ...
                break;
            default:
                fprintf(stderr, "ERROR: Unrecognized argument; only ./lab1b [--port=port] [--encrypt=filename] accepted\n");
                exit(1);
                break;
        }
    }
